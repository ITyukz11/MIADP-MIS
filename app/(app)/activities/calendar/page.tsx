...
import {
  formatDate,
  formatTime,
} from "@/components/table/data/activities/coa-columns";
...
const page = () => {
  ...
  const formatEndDateToISOWithoutTimezone = (
    endDate: Date,
    startDate: Date
  ) => {
    ...
    const fetchData = () => {
      try {
        if (activitiesData) {
          ...
          const formattedData =
            filteredActivities &&
            filteredActivities.map((event: any) => {
  ...
  const handleEventClick = (info: any) => {
    const activity = activitiesData.filter(
      (activity: { id: any }) => activity.id === info.event.id
    );
  ...
  const handleEventClick2 = (id: any) => {
    const activity = activitiesData.filter(
      (activity: { id: any }) => activity.id === id
    );
    setActivityData(activity);
    setCalendarSheetOpen(true);
  };
  ...
  if (activityLoading)
    return (
      <div className="w-full flex justify-center">
        <div
          className="flex flex-col py-2 lg:flex-row gap-5 flex-wrap lg:flex-nowrap overflow-hidden w-full max-w-[1800px]"
          style={{
            ...(fullScreenCalendar ? { width: "100%" } : {}),
          }}
        >
          <div className="flex flex-col w-full lg:w-1/4 gap-4 justify-start items-center">
          ...
          <Card
            className="h-fit w-full min-h-[700px] mb-2 lg:w-3/4 overflow-x-auto scrollbar-thin scrollbar-track-rounded-full"
            style={
              fullScreenCalendar ? { width: "100%", height: "100%" } : undefined
            }
          >
            <CardContent className="p-8 flex flex-col gap-2">
              <Skeleton className="h-16 w-full" />
  ...
  return (
    <div className="w-full flex justify-center">
      ...
      >
        {!fullScreenCalendar && (
          <div className="flex flex-col w-full lg:w-1/4 gap-4 justify-start items-center">
            <Card className="overflow-y-auto w-full lg:max-h-[400px] rounded-xl scrollbar-thin scrollbar-track-rounded-full">
                ...
                <CardTitle className="flex flex-row items-center justify-start gap-10">
                  ...
                  <div className="flex flex-row gap-2 items-center">
                    <Label className="text-sm md:text-base font-semibold">
                      {filteredUpcomingEvents.slice(0, 10).length} Upcoming
                      Event{filteredUpcomingEvents.length > 1 ? "s" : ""}
                    </Label>
                ...
                {activityLoading ? (
                  ...
                  filteredUpcomingEvents
                    ...
                    .map((event: any) => {
                      ...
                      return (
                        <div
                          className="mt-3 hover:cursor-pointer z-10"
                          key={event.id}
                          onClick={() => handleEventClick2(event.id)}
                        >
                            ...
                            <div className="flex flex-col justify-center gap-1 w-full text-left text-xs">
                              ...
                              <div className="flex gap-3 flex-row flex-wrap">
                                ...
                                <Label className="font-extralight text-xs sm:text-sm">
                                  {startDate === endDate || startTimeString
                                    ? startDate
                                    : `${startDate} - ${endDate}`}
                                </Label>
                                <Label className="font-extralight text-xs sm:text-sm">
            ...
            <Card className="overflow-y-auto w-full lg:max-h-[400px] rounded-xl scrollbar-thin scrollbar-track-rounded-full">
              <CardHeader className="font-bold gap-2 p-3 sticky top-0 z-10 border-b bg-white dark:bg-gray-800">
                <CardTitle className="flex flex-row items-center justify-start gap-10">
                  ...
                  <div className="flex flex-row gap-2 items-center">
                    <Label className="text-sm md:text-base font-semibold">
                      {filteredOnGoingEvents.slice(0, 10).length} Ongoing Event
                      {filteredOnGoingEvents.length > 1 ? "s" : ""}
                    </Label>
              ...
              <CardContent>
                ...
                {activityLoading ? (
                  ...
                  filteredOnGoingEvents.map((event: any) => {
                    const startDate = formatDate(event.start);
                    const endDate = formatDate(subtractOneDay(event.end));
                    const minus1dayEndDate = formatDate(
        ...
        >
          <div
            ref={cardRef}
            className="flex justify-end flex-row gap-2 p-1 px-5 pt-2"
          >
            <Button
              disabled={activityLoading}
              onClick={toggleFullScreen}
              variant="link"
              className="flex-row justify-center gap-1 items-center hidden lg:flex"
            >
          ...
          <CardContent>
            <FullCalendar
              ...
              headerToolbar={{
                ...
                center: "title",
                right:
                  "list,timeGridDay,timeGridWeek,dayGridMonth,multiMonthYear",
              }}
      ...
      <CalendarSheet
        ...
        openSheet={calendarSheetOpen}
        closeCalendarSheet={() => setCalendarSheetOpen(!calendarSheetOpen)}
      />
    </div>
  );
};
