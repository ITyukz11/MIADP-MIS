...
export function IncomingRoutingSlipDialog({
  open,
  setClose,
  selectedRow,
}: IncomingRoutingSlipProps) {
  ...
  const onSubmit = async (
    values: z.infer<typeof CorrespondenceIncomingSchema>
  ) => {
    ...
    <Dialog open={open} onOpenChange={setClose}>
      <DialogContent className="max-w-[750px] w-full overflow-y-auto scrollbar-thin max-h-[95vh] rounded-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Please encode your routing slip here and ensure to send it once
            completed.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          ...
          >
            <div className="grid grid-col gap-2 dark:bg-slate-600 bg-slate-300 rounded-md p-3">
              <Accordion type="single" collapsible>
                <AccordionItem value="item-1">
                  <AccordionTrigger className="p-0 font-semi-bold text-base mb-2">
                    <div>
                      Subject:{" "}
                      <Badge variant={"secondary"}>{selectedRow.subject}</Badge>{" "}
                    </div>
                  </AccordionTrigger>
              ...
              <div className="flex flex-wrap sm:grid grid-cols-3 gap-2 ">
                <Label>
                  No: <Badge variant={"secondary"}>DTS-01</Badge>
                </Label>
                <Label>
                  Date: <Badge variant={"secondary"}>{getTodayDate()}</Badge>
                </Label>
                <Label>
                  Sender:{" "}
                  <Badge variant={"secondary"}>{currentUser?.user.name}</Badge>
                </Label>
              </div>
              <Separator />
            </div>
