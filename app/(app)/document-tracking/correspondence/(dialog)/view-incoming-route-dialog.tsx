...
import { Forward, Send, Upload } from "lucide-react";
...
export function ViewIncomingRoute({ open, setClose, selectedRow }: ViewIncomingRouteProps) {
    ...
    const filteredDocumentData = documentsData && documentsData.filter((document) => document.id === selectedRow.documentId)
        ...
        <Dialog open={open} onOpenChange={setClose}>
            <DialogContent className="max-w-[750px] w-full overflow-y-auto scrollbar-thin max-h-[95vh] rounded-lg">
                      ...
                      <Skeleton className="h-24 w-full" />
                       ...
                       <AccordionItem value="item-1">
                           <AccordionTrigger className="p-0 font-semi-bold text-base mb-2">
                               <div>Subject: <Badge variant={'secondary'}>{filteredDocumentData[0]?.subject}</Badge> </div>
                           </AccordionTrigger>
                           <AccordionContent className="grid grid-col gap-2 pt-2">
                               {RenderDocumentInfo("Description:", filteredDocumentData[0]?.description)}
                               {RenderDocumentInfo("From:", filteredDocumentData[0]?.from)}
                               {RenderDocumentInfo("Encoder:", filteredDocumentData[0]?.encoder)}
                               {RenderDocumentInfo("Link:", filteredDocumentData[0]?.link)}
                               {RenderDocumentInfo("Date:", filteredDocumentData[0]?.date)}
                               {RenderDocumentInfo("Remarks:", filteredDocumentData[0]?.remarks)}
                           </AccordionContent>
                       </AccordionItem>
                   </Accordion>
                    }
                ...
                <DialogFooter className="flex justify-between">
                    <Button type="button">Forward To <Forward size={17} /></Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
