...
export async function GET(request) {
  try {
    ...
    const activities = await prisma.calendarOfActivity.findMany({
      where: {
        individualActivity: false, // Filter only activities where individualActivity is false
      },
      include: {
        user: {
          select: {
    ...
    const countsByRegion = activities.reduce((acc, activity) => {
      const region = activity.user?.region; // Handle case where user or region might be null
      if (region) {
        if (!acc[region]) {
          acc[region] = 0;
        }
