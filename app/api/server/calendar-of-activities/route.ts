...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const ongoingUpdate = await prisma.calendarOfActivity.updateMany({
      where: {
        dateFrom: { lte: today },
        dateTo: { gte: today },
    ...
    console.log(`Updated to Ongoing: ${ongoingUpdate.count} activities`);

    // Update to "Completed" if today is after dateTo, excluding Postponed and Cancelled
    const completedUpdate = await prisma.calendarOfActivity.updateMany({
      where: {
        dateTo: { lt: today },
    ...
    console.log(`Updated to Completed: ${completedUpdate.count} activities`);
