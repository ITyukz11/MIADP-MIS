...
function GenerateCodeDialog() {
    ...
    const onSubmit = async (values: z.infer<typeof GenerateCodeSchema>) => {
        ...
        try {
            ...
            const component = codeComponent.find((item) => item.value === values.component)?.label || values.component
            // Fetch the count of subproject codes from the API
            const response = await fetch(`/api/subproject/count?component=${component}`, {
                method: 'GET',
                headers: {
            ...
            values.province = codeProvince.find((item) => item.value === values.province)?.label || values.province;
            values.region = codeRegion.find((item) => item.value === values.region)?.label || values.region;
    ...
    const filteredAncestralDomainLoc = codeAncestralDomain.filter(AD => AD.region === selectedRegion)
