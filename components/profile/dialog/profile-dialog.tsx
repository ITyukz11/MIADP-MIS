...
export function ProfileDialog({ open, setClose }: ProfileDialogProps) {
  ...
  const [component4, setComponent4] = useState<boolean>(
  ...
  const form = useForm<z.infer<typeof UserSchema>>({
    ...
    defaultValues: {
      ...
      position: currentUser?.user.position || "",
      unit: currentUser?.user.unit || "",
      component: currentUser?.user.component || "",
      region: currentUser?.user.region || "",
      //   color: currentUser?.user.color || "",
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof UserSchema>) => {
    ...
    if (currentUser?.user.name !== values.name) {
      changes.push("name");
    }
    if (currentUser?.user.email !== values.email) {
      changes.push("email");
    }
    if (currentUser?.user.position !== values.position) {
      changes.push("position");
    }
    if (currentUser?.user.unit !== values.unit) {
      changes.push("unit");
    }
    if (currentUser?.user.component !== values.component) {
      changes.push("component");
    }
    if (currentUser?.user.region !== values.region) {
      changes.push("region");
    }
    ...
    try {
      await updateprofile(values, logs).then((data) => {
  ...
  return (
    <>
            ...
            >
              <div className="grid gap-1 pb-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="grid grid-cols-4 items-center gap-4">
                      <FormLabel className="text-xs sm:text-sm text-right flex justify-end">
                        Name
                        <FormMessage />
                      </FormLabel>
                      <FormControl>
                    ...
                    <FormItem className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Select
                        onValueChange={(newValue) => {
                          ...
                          newValue == "Component 4"
                            ? handleUnitDropDown(false)
                            : handleUnitDropDown(true);
                        }}
                        defaultValue={field.value}
                        disabled={editProfile || loading}
                      >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="grid grid-cols-4 items-center gap-4">
                      <FormLabel className="text-xs sm:text-sm text-right">
                        Region <FormMessage />
                      </FormLabel>
                      <Select
                        onValueChange={field.onChange}
                ...
                {!editProfile ? (
                  <>
                    <Button type="submit" disabled={loading}>
                      {" "}
                      {loading ? <LoadingSpinner /> : "Submit changes"}
                    </Button>
                  ...
                  <>
                    <Button
                      variant="secondary"
                      type="button"
                      onClick={() => setEnterPassDialog(!enterPassDialog)}
                    >
                      Change password
                    </Button>
      ...
      <EnterPasswordDialog
        open={enterPassDialog}
        setClose={() => setEnterPassDialog(!enterPassDialog)}
      />
    </>
  );
}
